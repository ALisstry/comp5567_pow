...
def FLAG():
    ...
    try:
        ...
        return f'Something went ERROR: {str(e)}, please contact admin of DDCTF to get your flag'
...
def new_app_route(url_pattern, **kwargs):
    def __dummy(f):
        ...
        # 为每个路由生成唯一函数名
        _.__name__ = f"view_{uuid.uuid4().hex}"
...
def sign_input_utxo(input_utxo_id, tx_hash, privkey):
    # Sign a message that binds the input utxo id to the transaction hash
    # This prevents replaying a signature from one transaction onto another
    if isinstance(input_utxo_id, str):
        input_utxo_id = input_utxo_id.encode('utf-8')
    if isinstance(tx_hash, str):
        tx_hash = tx_hash.encode('utf-8')
    message = tx_hash + b':' + input_utxo_id
    return rsa.sign(message, privkey, 'SHA-1').hex()
...
def create_tx(input_utxo_ids, output_utxo, privkey_from=None):
    # Create tx and bind signatures to the tx hash to avoid replay attacks
    tx = {'input': input_utxo_ids, 'signature': [], 'output': output_utxo}
    tx['hash'] = hash_tx(tx)
    if privkey_from is not None and len(input_utxo_ids) > 0:
        tx['signature'] = [sign_input_utxo(i, tx['hash'], privkey_from) for i in input_utxo_ids]
    else:
        tx['signature'] = ["" for _ in input_utxo_ids]
    return tx
...
def verify_utxo_signature(address, utxo_id, signature, tx_hash):
    try:
        if isinstance(utxo_id, str):
            utxo_id = utxo_id.encode('utf-8')
        if isinstance(tx_hash, str):
            tx_hash = tx_hash.encode('utf-8')
        message = tx_hash + b':' + utxo_id
        return rsa.verify(message, bytes.fromhex(signature), addr_to_pubkey(address))
    except:
        return False
...
def append_block(block, difficulty=int('f'*64, 16)):
    ...
    for tx in block['transactions']:
        ...
        # compute transaction hash early so signatures can be bound to this tx
        tx['hash'] = hash_tx(tx)
        for utxo_id, signature in zip(tx['input'], tx['signature']):
            ...
            if not verify_utxo_signature(utxo['addr'], utxo_id, signature, tx['hash']):
...
@app.route(url_prefix+'/test_height')
def test_height():
    init()
    tail = find_blockchain_tail()
    return f"Blockchain height: {tail['height']}, Total blocks: {len(session['blocks'])}"
